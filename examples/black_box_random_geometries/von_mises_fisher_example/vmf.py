...
class IveFunction(torch.autograd.Function):
    ...
    def backward(self, grad_output):
        ...
        return (
            None,
            grad_output
            * (IveFunction.apply(self.v - 1, z) - IveFunction.apply(self.v, z) * (self.v + z) / z),
        )


class VonMisesFisher(torch.distributions.Distribution):
    """
    A custom implementation of the von Mises-Fisher distribution
    over the sphere.
    """
    ...
    def rsample(self, shape=torch.Size()):
        ...
        x = torch.cat((w.unsqueeze(-1), w_.unsqueeze(-1) * v), -1)
    ...
    def __sample_w3(self, shape):
        ...
        self.__w = (
            1
            + torch.stack([torch.log(u), torch.log(1 - u) - 2 * self.scale], dim=0).logsumexp(0) / self.scale
        )
        return self.__w
    ...
    def entropy(self):
        output = -self.scale * self.ive(self.__m / 2, self.scale) / self.ive((self.__m / 2) - 1, self.scale)
    ...
    def _mylog_unnormalized_prob(self, x):
        output = self.scale.unsqueeze(-1) * (self.loc * x).sum(-1, keepdim=True)

        return output.view(*(output.shape[:-1]))

    def _mylog_normalization(self):
        output = -(
            (self.__m / 2 - 1) * torch.log(self.scale)
            - (self.__m / 2) * math.log(2 * math.pi)
            - (self.scale + torch.log(self.ive(self.__m / 2 - 1, self.scale)))
