...
class VAE_bivariate(nn.Module):
    def __init__(self, latent_size, device="cpu", learning_rate=1e-4):
        super(VAE_bivariate, self).__init__()
        self.latent_size = latent_size
        self.enc_mu = nn.Sequential(nn.Linear(2, 50),
                                    nn.Tanh(),
                                    nn.Linear(50, latent_size))

        self.enc_var = nn.Sequential(nn.Linear(2, 50),
                                    nn.Tanh(),
                                    nn.Linear(50, latent_size),
                                    nn.Softplus())

        self.dec_mu = nn.Sequential(nn.Linear(latent_size, 50),
                                    nn.Tanh(),
                                    nn.Linear(50, 2))

        self.dec_var = nn.Sequential(nn.Linear(latent_size, 50),
                                    nn.Tanh(),
                                    nn.Linear(50, 2),
                                    nn.Softplus())
